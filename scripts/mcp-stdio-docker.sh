...
SEARCH_ROUTE_WARMUP_ENGINES="${SEARCH_ROUTE_WARMUP_ENGINES:-duckduckgo_api,google_cb,google_lp,bing_lp,duckduckgo_cb,bing_cb}"
...
if ! docker ps -a --format '{{.Names}}' | grep -Fx "$CONTAINER" >/dev/null 2>&1; then
  docker create \
    ...
    -e SEARCH_ROUTE_WARMUP_ENGINES="$SEARCH_ROUTE_WARMUP_ENGINES" \
